...
class GuidesController extends MembersController {
    ...
    public function loadLists() {
        ...
        foreach ($purchase_orders as $row) {
            $this->list_purchase_orders[] = array('id' => $row['id_purchase_order'],
                "columns" => array(
                    $row['number'],
                    $row['provider_name']
                )
        ...
        if (isset($this->_id_guide)) {
            $materials = $this->_Model->listMaterials(array(
                'id_guide' => $this->_id_guide)
            );
            foreach ($materials as $row) {
                $this->list_materials[] = array('id' => $row['id_purchase_order_detail'],
                    "columns" => array(
                        $row['name'],
