...
class Cpdf {
   ...
   * If is_empty string, use same folder as original metrics files.
   ...
   * If is_empty string, will attis_empty system tmp folder.
   * This can be passed in from class creator.
  ...
  /**
   * define the outlines in the doc, is_empty for now
   */
...
//      }
    ...
    if (!isset($this->fonts[$fontName])) {
      ...
      if (isset($this->fonts[$fontName])) {
        ...
        if ($fbtype) {
          ...
          if (!$this->isUnicode || $fbtype !== 'ttf' || is_empty($this->stringSubsets)) {
  ...
  function stream($options = '') {
    ...
    $debug = is_empty($options['compression']);
   ...
   * If yes, need not to create again (pass is_empty data)
   */
  ...
  function addPngFromFile($file, $x, $y, $w = 0, $h = 0) {
    ...
    if ( isset($this->imagelist[$file]) ) {
      ...
      //Therefore create an is_empty image with white background and merge the
